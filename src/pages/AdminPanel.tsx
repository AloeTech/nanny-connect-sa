...
import { Shield, Users, Video, FileCheck, CreditCard, CheckCircle, X, Upload, Edit, Heart, Eye, Trash2, Plus, FileText, Film, Mail, Phone, Clock } from 'lucide-react';
...
export default function AdminPanel() {
  ...
  const updateDocumentStatus = async (nannyId: string, field: string, status: string) => {
    try {
      ...
      if (nanny && status === 'approved') {
        const documentType = field === 'criminal_check_status' ? 'Criminal Check' :
                          field === 'credit_check_status' ? 'Credit Check' :
                          field === 'proof_of_residence_status' ? 'Proof of Residence' : 'Document';
  ...
  // UPDATED: Handle interest action with dedicated PHP endpoint
  const handleInterestAction = async (interestId: string, action: 'approve' | 'decline') => {
    ...
    try {
      ...
      toast({ 
        title: "Success", 
        description: `Interest ${action}d${clientEmailResult.success || nannyEmailResult.success ? ' (emails sent)' : ' (email sending failed)'}` 
      });
    } catch (error: any) {
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                Nanny Management ({nannies.length})
              </CardTitle>
                    ...
                    <div className="flex flex-col lg:flex-row justify-between items-start gap-6">
                      ...
                      <div className="flex flex-col gap-3 min-w-[200px]">
                        ...
                        <Button
                          ...
                          variant="outline"
                          onClick={() => {
                            const subject = `Regarding your Nanny Profile - ${nanny.profiles.first_name}`;
                            const body = `Dear ${nanny.profiles.first_name},\n\n`;
                            window.location.href = `mailto:${nanny.profiles.email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
                          }}
                        >
                        ...
                        {nanny.profiles.phone && (
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => window.location.href = `tel:${nanny.profiles.phone}`}
                          >
                        ...
                        <div className="flex-1 space-y-4">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                              ...
                              <p><strong>Submitted:</strong> {new Date(interest.created_at).toLocaleString('en-ZA', {
                                dateStyle: 'medium',
                                timeStyle: 'short'
                              })}</p>
                            </div>
                            ...
                            <div className="space-y-2">
                              ...
                              <p><strong>Status:</strong> <Badge className={
                                interest.status === 'approved' ? 'bg-green-500' :
                                interest.status === 'declined' ? 'bg-red-500' : 'bg-yellow-500'
                              }>{interest.status.toUpperCase()}</Badge></p>
                            </div>
                          </div>
                      ...
                      <div className="flex flex-col lg:flex-row justify-between items-start gap-6">
                        <div className="flex-1 space-y-3">
                          <div className="flex items-start justify-between">
                            ...
                            <Badge variant={video.is_active ? "default" : "secondary"} className={
                              video.is_active ? "bg-green-500" : "bg-gray-500"
                            }>
                        ...
                        <CardContent className="p-6">
                          <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                            ...
                            <div className="text-right">
                              ...
                              <Badge variant={payment.status === 'completed' ? 'default' : 'secondary'} className={
